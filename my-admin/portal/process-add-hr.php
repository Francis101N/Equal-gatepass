...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        // Prevent Duplicate Entries by checking if the email is already registered
        $check_stmt = $pdo->prepare("SELECT id FROM hr WHERE email = ? LIMIT 1");
        $check_stmt->execute([$email]);
        if ($check_stmt->fetch()) {
            $_SESSION['error_message'] = "Conflict Error: An HR administrator profile with this email address already exists.";
            header("Location: add-hr.php");
            exit;
        }
        ...
        if (isset($_FILES['passport']) && $_FILES['passport']['error'] === UPLOAD_ERR_OK) {
            ...
            $new_filename   = 'hr_new_' . time() . '_' . bin2hex(random_bytes(4)) . '.' . $file_extension;
        ...
        // 5. Execute Core Data Insertion Layout Map
        $insert_sql = "INSERT INTO hr (name, branch, email, phone, passport) 
        ...
        $_SESSION['success_message'] = "Success: New HR Administrator profile created successfully.";
        header("Location: add-hr.php");
        exit;
    } catch (Exception $e) {
        error_log("Profile Insertion Error: " . $e->getMessage());
