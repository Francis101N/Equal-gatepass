...
<head>
    ...
    <title>Hr Details | Equal Gate-Pass </title>
        ...
        <div class="admin-main">
            ...
            if (!empty($token_id)) {
                try {
                    ...
                    if ($raw_id !== false) {
                        // Fetch comprehensive row information matching decoded index from hr_table
                        $stmt = $pdo->prepare("SELECT * FROM hr WHERE id = ? LIMIT 1");
                        $stmt->execute([$raw_id]);
                        $hr_data = $stmt->fetch();
                    }
            ...
            // Redirect or show alternative UI if the HR user profile is invalid or not found
            if (!$hr_data) {
                echo "<div class='container mt-5'><div class='alert alert-danger'><i class='bi bi-exclamation-triangle'></i> HR Administrator profile record could not be retrieved. Ensure token mapping parameters are valid. <a href='manage-hr.php' class='alert-link'>Return to HR Directory</a></div></div>";
                exit;
            }
            ?>
                    ...
                    <section class="row g-3">
                        ...
                        <div class="col-12 col-xl-4">
                            <div class="panel h-100 text-center profile-card shadow-sm border p-3 bg-white">
                                <div class="profile-hero py-3 mt-4">
                                    <?php if (!empty($hr_data['passport'])): ?>
                                        <?php
                                        // Clean up any stray backslashes or leading slashes from the DB entry
                                        $image_src = ltrim(str_replace('\\', '/', $hr_data['passport']), '/');
                                        ?>
                                        <img class="avatar-img avatar-xl profile-photo rounded-circle border border-3 mb-3 shadow-sm"
                                            ...
                                            alt="<?php echo htmlspecialchars($hr_data['name']); ?>"
                                    ...
                                    <?php else: ?>
                                        <div class="d-flex align-items-center justify-content-center rounded-circle bg-secondary text-white mx-auto mb-3 shadow-sm" style="width:120px; height:120px; font-size: 2.5rem; font-weight: bold; background-color: #6c757d !important;">
                                            <?php echo htmlspecialchars(strtoupper(substr($hr_data['name'] ?? 'HR', 0, 2))); ?>
                                        </div>
                                    <?php endif; ?>

                                    <h2 class="h4 mb-1 fw-bold text-dark"><?php echo htmlspecialchars($hr_data['name']); ?></h2>

                                    <p class="text-muted mb-1 font-monospace small">
                                        <code class="text-secondary">Role: HR </code>
                                    </p>

                                    <?php if (!empty($hr_data['email'])): ?>
                                        <p class="text-muted small mb-1">
                                            <i class="bi bi-envelope text-xs me-1"></i><?php echo htmlspecialchars($hr_data['email']); ?>
                                        </p>
                                    <?php endif; ?>
                                    <p class="text-muted small mb-1">
                                        Branch : <?php echo htmlspecialchars($hr_data['branch']); ?>
                                    </p>
                                </div>
                                ...
                                <div class="info-list mt-2 text-start small">
                                    <div class="d-flex justify-content-between py-2 border-bottom">
                                        ...
                                        <strong class="font-monospace text-dark">#<?php echo htmlspecialchars($hr_data['id']); ?></strong>
                                    </div>
                                    <div class="d-flex justify-content-between py-2 border-bottom">
                                        <span class="text-muted fw-medium">Assigned Office Branch:</span>
                                        <strong><i class="bi bi-geo-alt me-1"></i><?php echo htmlspecialchars($hr_data['branch']); ?></strong>
                                    </div>
                                    <div class="d-flex justify-content-between py-2 border-bottom">
                                        <span class="text-muted fw-medium">Phone Extension:</span>
                                        <strong class="font-monospace"><?php echo htmlspecialchars($hr_data['phone'] ?? 'N/A'); ?></strong>
                                    </div>
                                    <div class="d-flex justify-content-between py-2 pt-2">
                                        <span class="text-muted fw-medium">Onboarding Timestamp:</span>
                                        <strong class="text-dark"><?php echo !empty($hr_data['date_created']) ? date('Y-m-d', strtotime($hr_data['date_created'])) : 'N/A'; ?></strong>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-12 col-xl-8">
                            ...
                            <div class="panel mb-3 shadow-sm border p-3 bg-white">
                                ...
                                <form action="process_hr_update.php" method="POST" enctype="multipart/form-data" class="mt-3">
                                    ...
                                    <div class="row g-3">
                                        <div class="col-md-6">
                                            <label for="hr_name" class="form-label small fw-semibold text-dark">Full Legal Name</label>
                                            <input type="text" class="form-control form-control-sm" name="name" id="hr_name" value="<?php echo htmlspecialchars($hr_data['name'] ?? ''); ?>" required>
                                        </div>

                                        <div class="col-md-6">
                                            <label for="hr_branch" class="form-label small fw-semibold text-dark">Assigned Jurisdiction Branch</label>
                                            <input type="text" class="form-control form-control-sm" name="branch" id="hr_branch" value="<?php echo htmlspecialchars($hr_data['branch'] ?? ''); ?>" placeholder="e.g. Lagos Terminal" required>
                                        </div>

                                        <div class="col-md-6">
                                            <label for="hr_email" class="form-label small fw-semibold text-dark">Email Routing Target Address</label>
                                            <input type="email" class="form-control form-control-sm" name="email" id="hr_email" value="<?php echo htmlspecialchars($hr_data['email'] ?? ''); ?>" required>
                                        </div>

                                        <div class="col-md-6">
                                            <label for="hr_phone" class="form-label small fw-semibold text-dark">Contact Phone Number</label>
                                            <input type="text" class="form-control form-control-sm" name="phone" id="hr_phone" value="<?php echo htmlspecialchars($hr_data['phone'] ?? ''); ?>" required>
                                        </div>

                                        <div class="col-12">
                                            <label for="hr_passport" class="form-label small fw-semibold text-dark">Replace Passport Photo (Optional)</label>
                                            <input type="file" class="form-control form-control-sm" name="passport" id="hr_passport" accept="image/*">
                                ...
                                <div class="activity-list border-start ps-3 ms-2 position-relative">
                                    <div class="activity-item pb-3 mb-2 position-relative">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-muted mb-0 small">
                                                This administrator profile account is verified and fully authorized to execute gate pass application choices for the <strong><?php echo htmlspecialchars($hr_data['branch']); ?></strong> workstation database node maps.
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="activity-item position-relative">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-muted small mb-0 font-monospace">
                                                <i class="bi bi-calendar-event"></i> Initial Onboard Timestamp: <?php echo !empty($hr_data['date_created']) ? htmlspecialchars(date('M d, Y - h:i A', strtotime($hr_data['date_created']))) : 'N/A'; ?>
                                            </p>
                                        </div>
                                    </div>
                                </div>
