...
// 1. Authentication Guard Check (Only HR roles can edit/update security officer profiles)
if (!isset($_SESSION['admin_role']) || $_SESSION['admin_role'] !== 'hr') {
    ...
    header("Location: security-details.php");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($branch) || !$email || empty($phone)) {
        ...
        header("Location: security-details.php?id=" . urlencode($token_id));
        exit;
    }

    try {
        // Fetch existing record configuration map from security table to preserve previous passport files
        $fetch_stmt = $pdo->prepare("SELECT passport FROM security WHERE id = ? LIMIT 1");
        $fetch_stmt->execute([$security_id]);
        $existing_record = $fetch_stmt->fetch();

        if (!$existing_record) {
            $_SESSION['error_message'] = "Processing Error: Targeted security profile row index does not exist.";
            header("Location: security-details.php?id=" . urlencode($token_id));
            exit;
        }
        ...
        if (isset($_FILES['passport']) && $_FILES['passport']['error'] === UPLOAD_ERR_OK) {
            ...
            $new_filename   = 'security_' . $security_id . '_' . time() . '_' . bin2hex(random_bytes(4)) . '.' . $file_extension;
        ...
        // 5. Execute Update Statements onto security table
        $update_sql = "UPDATE security 
                       SET name     = :name, 
                           branch   = :branch, 
                           email    = :email, 
                           phone    = :phone, 
        ...
        $_SESSION['success_message'] = "Success: Security Officer Profile details updated successfully.";
        header("Location: security-details.php?id=" . urlencode($token_id));
        exit;
    } catch (Exception $e) {
        error_log("Security Profile Update Error: " . $e->getMessage());
